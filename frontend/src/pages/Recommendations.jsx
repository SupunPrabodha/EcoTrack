		...
		queryFn: async () =>
			(
				await api.get("/recommendations", {
					params: { page, limit, ...(search ? { search } : {}), ...(impact ? { impact } : {}) },
				})
			).data,
		refetchOnWindowFocus: false,
		retry: 1,
			...
			<div className="max-w-7xl mx-auto px-4 py-8 space-y-6">
						...
						<div className="flex items-end">
							...
							>
								<span className="inline-flex items-center justify-center gap-2">
									{recQ.isFetching ? (
										<IconRefresh width={18} height={18} className="animate-spin" />
									) : (
				...
				{!recQ.isLoading && recQ.data?.weather && (
					<Card title="Weather Context">
						...
						<div className="text-sm text-slate-400">Condition: {recQ.data.weather.condition}</div>
					</Card>
				)}
				...
				{!recQ.isLoading &&
					recQ.data?.tips?.map((tip, i) => (
						<Card key={`${tip.title}-${i}`} title={tip.title}>
								...
								{(r?.context?.range?.from || r?.context?.weather?.condition) && (
									<div className="mt-3 text-xs text-slate-500">
										...
										{r?.context?.weather?.condition ? (
											<span>
												{r?.context?.range?.from && r?.context?.range?.to ? " · " : ""}
												Weather: {r.context.weather.condition}
												{typeof r.context.weather.tempC === "number" ? `, ${r.context.weather.tempC}°C` : ""}
												{r.context.weather.city ? ` (${r.context.weather.city})` : ""}
											</span>
										) : null}
									</div>
								)}
